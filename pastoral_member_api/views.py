...
from .serializers import FindPastoralMemberSerializer, ListPastoralMemberSerializer, SavePastoralMemberSerializer, UpdatePastoralMemberSerializer
...
class PastoralMemberRepository:
    def get_pastoral_member(self, pk: int):
        try:
            return PastoralMember.objects.get(id=pk)
        except PastoralMember.DoesNotExist:
            return None


class SavePastoralMemberView(generics.CreateAPIView):
    #queryset = m.PastoralMember.objects.all()
    error_message = ErrorMessage
    serializer_class = SavePastoralMemberSerializer
...
class ListPastoralMemberView(generics.ListAPIView):
    queryset = PastoralMember.objects.all()
    serializer_class = ListPastoralMemberSerializer
    permission_classes = [permissions.AllowAny]
    error_message = ErrorMessage
...
class UpdatePastoralMemberView(generics.UpdateAPIView):
    error_message = ErrorMessage
    queryset = PastoralMember.objects.all()
    serializer_class = UpdatePastoralMemberSerializer
    permission_classes = [permissions.AllowAny]
    repository = PastoralMemberRepository()
    
    def update(self, request, id: int, **kwargs):
        OBJECTO = 'Membro da Pastoral'
        
        try:
            pastoral_member = self.repository.get_pastoral_member(id)
            
            if not pastoral_member:
            ...
            serializer = self.serializer_class(pastoral_member, data=request.data, partial = True)
...
class DeletePastoralMemberView(generics.DestroyAPIView):
    queryset = PastoralMember.objects.all()
    serializer_class = ListPastoralMemberSerializer
    permission_classes = [permissions.AllowAny]
    error_message = ErrorMessage
    ...
    def delete(self, request, id: int, **kwargs):
        ...
        try:
            ...
            if not pastoral_member:
                logger.info(self.error_message.not_found_log(self, OBJECTO, id))
                return Response({'message':self.error_message.not_found(self, OBJECTO)}, status=status.HTTP_404_NOT_FOUND)
            serializer = self.serializer_class(pastoral_member)
            pastoral_member.delete()
...
class FindPastoralMemberView(generics.ListAPIView):
    queryset = PastoralMember.objects.all()
    serializer_class = FindPastoralMemberSerializer
    permission_classes = [permissions.AllowAny]
    error_message = ErrorMessage
    repository = PastoralMemberRepository()
