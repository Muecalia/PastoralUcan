...
from .serializers import ListPastoralVisitorSerializer, SavePastoralVisitorSerializer, UpdatePastoralVisitorSerializer
...
class PastoralVisitorRepository:
    def get_pastoral_member(self, pk: int):
        try:
            return PastoralVisitor.objects.get(id=pk)
        except PastoralVisitor.DoesNotExist:
            return None


class SavePastoralVisitorView(generics.CreateAPIView):
    #queryset = m.PastoralVisitor.objects.all()
    error_message = ErrorMessage
    serializer_class = SavePastoralVisitorSerializer
...
class ListPastoralVisitorView(generics.ListAPIView):
    queryset = PastoralVisitor.objects.all()
    serializer_class = ListPastoralVisitorSerializer
    permission_classes = [permissions.AllowAny]
    error_message = ErrorMessage
...
class UpdatePastoralVisitorView(generics.UpdateAPIView):
    error_message = ErrorMessage
    queryset = PastoralVisitor.objects.all()
    serializer_class = UpdatePastoralVisitorSerializer
    permission_classes = [permissions.AllowAny]
    repository = PastoralVisitorRepository()
    
    def update(self, request, id: int, **kwargs):
        OBJECTO = 'Visitante da Pastoral'
        
        try:
            pastoral_visitor = self.repository.get_pastoral_member(id)
            
            if not pastoral_visitor:
            ...
            serializer = self.serializer_class(pastoral_visitor, data=request.data, partial = True)
...
class DeletePastoralVisitorView(generics.DestroyAPIView):
    queryset = PastoralVisitor.objects.all()
    serializer_class = ListPastoralVisitorSerializer
    permission_classes = [permissions.AllowAny]
    error_message = ErrorMessage
    ...
    def delete(self, request, id: int, **kwargs):
        ...
        try:
            ...
            if not pastoral_visitor:
                logger.info(self.error_message.not_found_log(self, OBJECTO, id))
                return Response({'message':self.error_message.not_found(self, OBJECTO)}, status=status.HTTP_404_NOT_FOUND)
            serializer = self.serializer_class(pastoral_visitor)
            pastoral_visitor.delete()
...
class FindPastoralVisitorView(generics.ListAPIView):
    queryset = PastoralVisitor.objects.all()
    serializer_class = ListPastoralVisitorSerializer
    permission_classes = [permissions.AllowAny]
    error_message = ErrorMessage
